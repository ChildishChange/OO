...
public class RequestList 
{
	...
	public void move(int i)
	{
		...
		temp = this.list[i];
		for(int n = i;n>this.Front;n--)
		{
			this.list[n] = this.list[n-1];
		}
		this.list[this.Front] = temp;
	...
	/*	else if(Rear!=0&&this.list[Rear-1].getTime() == parameter[1])
		...
		if(judge[1]=='F') temp.setType(1);
		else if(judge[1]=='E') temp.setType(2);
		
		if(S.indexOf("D")!=-1) temp.setUD(-1);
		else if(S.indexOf("U")!=-1) temp.setUD(1);
