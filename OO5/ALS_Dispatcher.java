...
public class ALS_Dispatcher extends Dispatcher 
{
	...
	public void ALS_ElevatorRun(RequestList L,Elevator E)
	{
		...
		while(!L.IsEmpty())
		{
			...
			{
				if(true/*temp.judge(temp,L.getList(i),TimeOfDispatch,E)*/)
				{
			...
			while(true)//找直到找不
			{
				...
				for(int i = L.getFront();i<L.getRear();i++)
				{
					if(true/*index[0]==-1&&temp.judge(temp,L.getList(i),TimeOfDispatch,E)*/)
					{
				...
				if(j == -1)
				{
					E.Goto(temp.getFloor(),temp);
					
					break;
				}
				//现在temp2是min
				E.Goto(temp2.getFloor(),temp);
